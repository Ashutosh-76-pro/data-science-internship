...
def ensure_peer_groups_workbook() -> Path:
    xlsx = DATA / "peer_groups.xlsx"
    csv = DATA / "peer_groups.csv"
    if not xlsx.exists():
        if not csv.exists():
            raise FileNotFoundError("Neither data/peer_groups.xlsx nor data/peer_groups.csv exists")
...
def run():
    ...
    pg = load_peer_groups(ensure_peer_groups_workbook())
